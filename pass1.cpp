...
void evaluateExpression(string expression, bool &relative, string &tempOperand, int lineNumber, ofstream &errorFile, bool &error_flag)
{
  ...
  for (int i = 0; i < expression.length();)
  {
    ...
    {
      writeData = "Line: " + to_string(lineNumber) + " : cant find the symbol error " + singleOperand;
    ...
    {
      writeData = "Line: " + to_string(lineNumber) + " : invalid expression";
      writeToFile(errorFile, writeData);
      error_flag = true;
      Illegal = true;
      break;
    ...
    {
      writeData = "at line " + to_string(lineNumber) + " : invalid operator exists in the operation " + singleOperator;
      writeToFile(errorFile, writeData);
      error_flag = true;
      Illegal = true;
      break;
  ...
  if (!Illegal)
  {
    ...
    {
      writeData = "at line  " + to_string(lineNumber) + " : invalid expression";
      writeToFile(errorFile, writeData);
      error_flag = true;
      tempOperand = "00000";
      relative = 0;
...
void pass1()
{
  ...
  if (!intermediateFile)
  {
    cout << "cant open the file : intermediate = " << fileName << endl;
    exit(1);
  }
  ...
  if (!errorFile)
  {
    cout << "cant open the file: error = " << fileName << endl;
    exit(1);
  }
    ...
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (!checkCommentLine(fileLine))
      {
        if (label != "")
        {
          ...
          {
            writeData = "at line " + to_string(lineNumber) + " : duplice symbol exist '" + label + "'. initially defined for " + SYMTAB[label].address;
            writeToFile(errorFile, writeData);
            error_flag = true;
          }
        }
        ...
        {
          readFirstNonWhiteSpace(fileLine, index, statusCode, operand);
          writeData = "at line: " + to_string(lineNumber) + " : wrong opcode " + opcode;
          writeToFile(errorFile, writeData);
          error_flag = true;
        }
        readFirstNonWhiteSpace(fileLine, index, statusCode, comment, true);
